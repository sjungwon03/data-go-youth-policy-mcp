...
- 청년정책 코드값 레퍼런스 조회: `youth_policy_get_code_reference`
...
- `youth_policy_get_code_reference`
...
응답 코드값 해석이 필요하면 youth_policy_get_code_reference를 사용한다.
...
```python
["youth_policy_search", "youth_policy_get_detail", "youth_policy_get_code_reference", "youth_policy_get_raw"]
```
...
응답에 포함되는 제공기관, 제공방법, 승인상태, 신청기간, 사업기간, 결혼상태, 소득조건, 전공, 취업, 학력, 특화요건, 정책분류, 정책키워드 코드값은 `youth_policy_get_code_reference`로 조회할 수 있습니다. 문서 표는 [청년정책 API 참고](docs/api-reference.md#코드값-참고)를 확인하세요.
...
LLM 에이전트가 응답에 포함된 `pvsnInstGroupCd`, `plcyPvsnMthdCd`, `plcyAprvSttsCd`, `aplyPrdSeCd`, `bizPrdSeCd`, `mrgSttsCd`, `earnCndSeCd`, `plcyMajorCd`, `jobCd`, `schoolCd`, `sbizCd`, `lclsfNm`, `mclsfNm`, `plcyKywdNm` 값을 사람이 읽을 수 있는 표시명으로 해석할 때 사용합니다.

입력:

- `group?: string` -> 코드 그룹명. 생략하면 전체 그룹을 반환합니다.
- `code?: string` -> 특정 코드값. `group`과 함께 지정하면 단일 코드 표시명을 반환합니다.

예시:

```json
{"group": "sbizCd", "code": "0014005"}
```
